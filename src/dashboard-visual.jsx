...
export default function DashboardVisual({ organization, userEmail, onGoCampaigns }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!active) return
        setState({ loading: false, error: '', leads, sales, messages, upcoming: contactsResult.data || [], weekly, clients: clientsResult.count || 0 })
      } catch (error) {
  ...
  const upcoming = showAllContacts ? state.upcoming : state.upcoming.slice(0, 5)
