...
export default function AdminHealthMonitor() {
  ...
  return <>
    ...
    <section className="panel"><span className="eyebrow">ALERTAS</span><h3>Destinatários e canais</h3>
      ...
      {alerts.length ? alerts.map((row, index) => <p key={index} className="muted">{formatMoment(row.created_at)} • {row.channel === 'email' ? 'E-mail' : 'WhatsApp'} • {row.delivery_status === 'sent' ? 'Aceito pelo provedor' : row.delivery_status === 'failed' ? 'Falhou' : 'Não configurado'} • {row.event === 'down' ? 'Queda' : row.event === 'test' ? 'Teste (sem incidente)' : 'Recuperação'}{row.detail ? ` — ${row.detail}` : ''}</p>) : <p className="muted">Nenhum alerta enviado ou registrado. Informe os destinatários e valide os canais do servidor.</p>}
    </section>
